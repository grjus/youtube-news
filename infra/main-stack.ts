...
import { awsSdkModuleName, MainTable, PROCESSING_MODE_INDEX, STATE_MACHINE_ARN_ATTR } from '../src/domain/consts'
...
export class MainStack extends Stack {
    constructor(
        ...
        const alarmMessageSenderFunction = new NodejsFunction(this, 'AlarmMessageSenderFunction', {
            ...
            handler: 'handler',
            entry: join('src', 'lambdas', 'alarm-message-sender.ts'),
        ...
        const scheduledNotificationPoller = lambdaFactory(this, {
            ...
            removalPolicy,
            retention: logRetention,
            entry: join('src', 'lambdas', 'scheduled-video-poller.ts'),
            handler: 'handler',
            environment: {
                TABLE_NAME: mainTable.tableName,
                SECRET_NAME: secret.secretName,
